...
export const getMetricsUrl = (since: Date) => {
  if (!METRICS_API_URL) {
    console.error(
      "REACT_APP_METRICS_API_URL environment variable is not defined"
    );
  }
...
// should be returned
interface ForecastBackendResponse<
  T extends BackendHillForecastRecord | BackendHuntForecastRecord,
> {
  success?: boolean;
...
export const fetchHillRecords = async (since: Date): Promise<BusynessData> => {
  try {
    const historicalResponse = await fetch(
      getHistoricalRecordsUrl("hill", since)
    );
    // unchecked cast
    ...
    if (!historicalBody.success) {
      throw new Error(
        "server was unable to process the request for historical records"
      );
    }
    ...
    const historicalRecords: HistoricalRecord[] = (
      historicalBody.busynessRecords ?? []
    ).map((record) => {
      return {
    ...
    if (!forecastBody.success) {
      throw new Error(
        "server was unable to process the request for forecast records"
      );
    }
    ...
    const forecastRecords: ForecastRecord[] = (
      forecastBody.busynessForecastRecords ?? []
    ).map((forecast) => {
      return {
        ...forecast,
...
export const fetchHuntRecords = async (since: Date): Promise<BusynessData> => {
  try {
    const historicalResponse = await fetch(
      getHistoricalRecordsUrl("hunt", since)
    );
    // unchecked cast
    const historicalBody =
...
export const fetchMetrics = async (since: Date): Promise<MetricsData> => {
  try {
    ...
    // unchecked cast
    const metricsBody =
      (await metricsResponse.json()) as MetricsBackendResponse;
    ...
    const metrics: LibraryMetrics[] = metricsBody.metrics.map((metric) => {
      return {
        ...
        records: metric.records.map((record) => {
          const { record_datetime, total_count_actual, total_count_predicted } =
            record;
          return {
            total: {
              count: {
